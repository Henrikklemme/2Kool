...
assets/
  ...
  i18n.js                 EN/DA translation dictionary + language-switch logic
  main.js                 contact-form mailto fallback + wires up the language toggle
...
- **The EN/DA toggle now actually translates the page.** All copy lives in
  `assets/i18n.js` as a plain `{ en, da }` dictionary — no build step, no
  framework. Clicking EN/DA swaps every piece of text on the page, updates
  the `<html lang>` attribute, and remembers the choice in `localStorage` for
  the next visit. To edit or add copy, just edit the strings in
  `assets/i18n.js`; each key corresponds to a `data-i18n="key"` attribute in
  `index.html`.
